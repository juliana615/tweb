...
import AppEditWordTab from './editWord';
import Row from '../../row';
...
export default class AppReservedWordsTab extends SliderSuperTab {
  ...
  public init(p: ReturnType<typeof AppReservedWordsTab['getInitArgs']> = AppReservedWordsTab.getInitArgs()) {
    this.container.classList.add('chat-folders-container');
    this.setTitle('WordList.Reserved.List.Title');
    console.log('WordList.Reserved.List.Title');
    ...
    i18n_({element: caption, key: 'WordList.Reserved.Header'});

    this.createFolderBtn = Button('btn-primary btn-color-primary btn-control', {
      text: 'WordList.Reserved.NewTitle',
      icon: 'add'
    });
    ...
    attachClickEvent(this.createFolderBtn, async() => {
      if(!(await this.canCreateFolder())) {
        ...
        this.slider.createTab(AppEditWordTab).open();
      }
    }, {listenerSetter: this.listenerSetter});
